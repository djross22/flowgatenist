...
//
// This version uses histogram binned data, and calculates the contribution to the log posterior
//   for the mid point of each bin, then adds that contibution multiplied by the event count for that bin 
//
// Also, in this version, the background and noise distributions are set.

data {
  int<lower=1> N;          // number of histogram bins for data
  vector[N] signal;        // mid-point flowcytometry signal for each bin
...
parameters {
  real log_difference;           // 1/2 difference between log alpha and log beta for gamma distribution
