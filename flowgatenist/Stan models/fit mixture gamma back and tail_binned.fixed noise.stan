...
parameters {
  real<lower=-log_lamb/2-0.2, upper=6> log_difference;  // 1/2 difference between log alpha and log_scaled_beta for gamma distribution
  real<lower=-4.5, upper=log_lamb/2+1.7> log_average;   // average of log alpha and log_scaled_beta for gamma dist.
...
generated quantities {
  ...
  real effective_tail_fraction;  // fraction of cells in tail, assuming tail is from multi-plet events
  real tail_fraction;            // number of cells in tail, assuming tail is from big cells
  ...
  tail_fraction = theta_tail/(theta_tail + theta1);
