...
transformed parameters {
  ...
  vector[N] gamma_log_like;        // log likelihood, gamma distribution part
  vector[N] back_log_like;        // log likelihood, background distribution part
  ...
  target_add = 0;
  for (i in 1:N) {
    ps[1] = log(theta[2]) + gamma_lpdf(gamma_signal[i] | alpha, beta); // singlet events
    
    for (k in 2:N_tail) {  // multi-plet/tail events
      ps[k] = log(theta[3]) - k*tail_B - log_norm_Z + gamma_lpdf(gamma_signal[i] | k*alpha, beta);
    }
    
    ps[N_tail+1] = log(theta[1]) + exponential_lpdf(gamma_signal[i] | lamb); // background/dark events
    
    log_like[i] = log_sum_exp(ps);
    gamma_log_like[i] = ps[1];
    back_log_like[i] = ps[N_tail+1];
